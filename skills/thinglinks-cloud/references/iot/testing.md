...
| **协议无关下行**(内部直连) | `POST <broker-service>/inner/deviceDownlinkOpen/dispatch` | `DownlinkCommand`:`protocolType`(MQTT/WEBSOCKET/TCP,空→兜底 MQTT)、`tenantId`、`clientId`(WS 必填)、`topic`、`qos`、`payload`、`expirySeconds`… | 按协议选 sender;`issueCommands` 内部走的就是它;经网关应拒绝 |
| **broker 裸 MQTT 发布**(内部直连) | `POST <broker-service>/inner/mqttBrokerOpen/sendMessage` | `PublishMessageRequestVO`:`tenantId`、`topic`、`qos`、`payload`、`forceBase64Decode` | 最底层,直发 BifroMQ;经网关应拒绝 |
...
| **调试历史** | `GET /link/deviceCommand/debugHistory` | `deviceIdentification`、`topic`、`limit` | MQTT/WS 调试台近 N 条下发/响应 |

## 四、端到端测试流

1. **模拟上行**:连设备 + 发 datas(见 `device-access.md`)—— 或先 `POST /mqs/transform/debug` 干跑转换。
2. **下发命令**:`issueCommands`(物模型命令)/ `sendMqttCustomMessage`(裸报文)/ 内部直连 `dispatch`。
