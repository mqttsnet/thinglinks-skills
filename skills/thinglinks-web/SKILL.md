...
description: >
  ...
  alarm notification editor, the personal-centre **MCP credential page** (the only place an AI
  client gets a ThingLinks MCP credential), or drawing flow diagrams (Excalidraw). Trigger whenever the user works
...
- 权限点匹配是**通配符**(`module:sub:action` / `module:*:action` / `*`);用 `usePermission().hasPermission`,
  或指令 —— **默认用 `v-hasAnyPermission`(代码库 102 处)而不是 `v-hasPermission`(6 处)**,
  两者在多权限点时语义相反(任意 vs 全部),详见 `references/routing-permission.md`。
...
| --- | --- | --- |
| [references/project-structure.md](references/project-structure.md) | 技术栈、`src/` 目录与 `views/` 顶层分域、个人中心 MCP 凭证页、构建脚本、多租户、i18n、store/字典 | 总体上手、找东西放哪、改 MCP 凭证页 |
...
> 📌 **最后核对**：2026-08-22，对照 `thinglinks-web-pro` 当前检出。
> 组件、路径和配置契约随版本演进，落地前核对当前源码与根目录产品清单。
